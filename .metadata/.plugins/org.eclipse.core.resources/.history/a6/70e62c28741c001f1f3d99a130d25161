...
public class usuario {
	...
	public boolean cambio_contraseña(int user,String new_pass) {
		 boolean res=false;
		 Conexion con=new Conexion();
		 String sql="Update tb_usuario set clave_us ='"+new_pass+
				 "' where id_us="+user;
		 try {
			 con.Ejecutar(sql);
			 res=true;
	...
	{
	String menu="<ul style=\"background: white; border: 1px solid black;\">";
	...
	{
	menu+=
			"<li><a href="+rs.getString(3)+" accesskey="+rs.getInt(1)+">"+rs.getString(2)+
	"</a></li>";
	}
	menu+="</ul>";
	}
